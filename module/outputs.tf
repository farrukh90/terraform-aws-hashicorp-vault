output "endpoints" {
  ...
  vault_2 (${aws_instance.vault-server[0].public_ip}) | internal: (${aws_instance.vault-server[0].private_ip})
    ...
    $ ssh -l ${var.user} ${aws_instance.vault-server[0].public_ip} -i ${var.key_name}.pem

    # Root token:
    $ ssh -l ${var.user} ${aws_instance.vault-server[0].public_ip} -i ${var.key_name}.pem "cat ~/root_token"
    # Recovery key:
    $ ssh -l ${var.user} ${aws_instance.vault-server[0].public_ip} -i ${var.key_name}.pem "cat ~/recovery_key"
  ...
  vault_3 (${aws_instance.vault-server[1].public_ip}) | internal: (${aws_instance.vault-server[1].private_ip})
    ...
    $ ssh -l ${var.user} ${aws_instance.vault-server[1].public_ip} -i ${var.key_name}.pem
